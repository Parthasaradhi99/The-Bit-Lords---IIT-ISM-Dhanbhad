...
for i in range (1,sheet_obj.max_row):
    names.append(sheet_obj.cell(row=i,column=1).value)'''
...
known_face_encodings=[0]*strength
known_face_names=[0]*strength
for j in range (strength):
    known_face_encodings[j]=encodingarray[j]
    known_face_names[j]=names[j]
